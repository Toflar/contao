...
export default class extends Controller {
    ...
    connect() {
        ...
        // Backwards compatibility for parent mode, will unhide the operation if no other drag handle is found (remove in Contao 6)
        for (const el of [...this.element.children]) {
            const handles = el.querySelectorAll('.drag-handle');

            // If there are less than two handles, we are not in BC mode
            if (handles.length <= 2) {
                continue;
            }
